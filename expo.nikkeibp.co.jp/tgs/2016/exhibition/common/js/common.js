...
//[topCheck]
var currentPage = "undefind";

if(document.getElementById("TopInfoModal")){
	currentPage = "TOP";
}
...
var modalCloseBtn = $(".ModalBlock .close");
//modalActionCatch
var modalCatch = $(".modalActionCatch");


var modalFolder = $("#TopInfoModal");
var modalContent = $(".ModalBlock");
...
function noScrollAdjust(){
	
	var myScroll = winScroll - tmpScroll;
	$("body").css({"margin-top":myScroll});
...
function modalOpen(){
	...
	$(modalFolder).css({"display":"block","opacity":0});
	
	$(modalFolder).animate({"opacity":1},500);
...
function modalClose(){
	...
	$(modalFolder).animate({"opacity":0},500,function(){
		//CB
		scrollFix = false;
		$(modalFolder).css({"display":"none","opacity":""});
		//$("body").css({"overflow":""});
		});
...
function modalAdjust(){
	...
	$(modalFolder).css({"height":winH});
	
	$(modalCatch).css({"height":winH});
	
	var myMT = (winH - $(modalContent).height())/2;
	
	$(modalContent).css({"margin-top":myMT});
}
