...
function bpPrivacyCookieBanner(){
  ...
  // 保存されている日時を取得
  const storageDate = window.localStorage.getItem('disablePrivacy_tgs_visitors_jp');
  if((new Date).getTime() - Date.parse(storageDate) <= 7776000000){
    ...
    document.querySelector('.bpPrivacy_button-close').addEventListener('click', ()=> {
      ...
      window.localStorage.setItem('disablePrivacy_tgs_visitors_jp', ''.concat(formatDate));
