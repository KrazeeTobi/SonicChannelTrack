...
(function($) {
  ...
  $(function() {
    ...
    $(window).on('load resize', function(){
      ...
      //headerHeight = (window.matchMedia('screen and (max-width:1170px)').matches)? 60 : 50;
    });
    ...
    $('a[href ^="#"]').click(function() {
      ...
      if (headerHeight == null) headerHeight = $('.js-header').innerHeight();
      let position = target.offset().top - headerHeight;
      const modal = $(this).hasClass('js-modal');
  ...
  $(window).on('load', function() {
    if(location.hash) {
      ...
      if (target.length && headerHeight != null) {
  ...
  $(function() {
    if($('.js-modal').length) {
      ...
      const hash = location.hash,
          hashExists = !!document.getElementById(hash.replace('#', ''));
      if(hash && hashExists) {
        $.magnificPopup.open({
          items: {src: hash},
          type: 'inline', 
  ...
  if($(scrollId).length) {
    ...
    $(window).on('load resize', function(){
      ...
      if(scrollWidth - clientWidth > 0) {
        $(scrollId).next('p').addClass('-show');
        if(clientHeight == offsetHeight) {
          $(scrollId).next('p').addClass('-sp');
        } else {
          $(scrollId).next('p').removeClass('-sp');
        }
      } else {
        $(scrollId).next('p').removeClass('-show');
      }
    });
    $(scrollId).on('scroll', function() {
      ...
      clientHeight = element.clientHeight;
      offsetHeight = element.offsetHeight;
      if(Math.floor(scrollWidth - (clientWidth + this.scrollLeft)) <= 0) {
        $(this).parents('div').addClass('-shadowOff');
      } else {
        $(this).parents('div').removeClass('-shadowOff');
      }
    });
  }
  ...
  // more
  // -----------------------------------------------------------------------------
  const moreId = '.js-more',
        showNo = 8;
  if($(moreId).length) {
    const target = $(moreId).prev('ul').children('li');
  ...
  // ----------------------------------------------------------------------------------------------------------------------
  $(window).on('load scroll', function() {
    let elem = $('.js-animation');
    const isAnimate = '-animate';
    const pageBottom = docH - winH;

    elem.each(function() {

      let elemOffset = $(this).offset().top;

      if (scrollTop > elemOffset - winH + (winH / 10)) {
    ...
    if (pageBottom <= scrollTop) {
      if(!$('.js-footer').hasClass(isAnimate)) {
        $('.js-footer').addClass(isAnimate);
      }
    }
  });

  $(window).on('load', function() {
    $('.js-mainBg').css({'opacity': 1});
  });
