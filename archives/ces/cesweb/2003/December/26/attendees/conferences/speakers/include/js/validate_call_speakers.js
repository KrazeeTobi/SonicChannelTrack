...
function validate_cfs_form(){
	...
	if (stringCity.length == 0) {
		alert ("Please enter the City for the speaker's address.");
		speakers_form.city.focus();
		return false;
	} 
	
	if (stringCountryValue == "unselected") {
		alert ("Please select the Country for the speaker's address.");
		document.speakers_form.country.focus();
		return false;
	}
	
	if (stringCountryValue == "United States" && stringStateValue == "unselected") {
		alert ("You indicated that the speaker's address is within the United States but have not selected a State. Please select a State.");
		document.speakers_form.state.focus();
		return false;
	}

	if (stringCountryValue == "United States" && stringZip.length == 0) {
		alert ("You indicated that the speaker's address is within the United States but have not entered the Zip Code. Please enter the Zip Code.");
	...
	if (stringTopics.length == 0 && otherTopic.length == 0) {
		alert ("Please check all topics the speaker is qualified to address. You must either select at least one topic or provide an additional topic in the box labeled 'Other'.");
		return false;
	}
	
	if (topicsCount > 3) {
		alert ("You have selected more than 3 topics that the speaker is qualified to address. Please select only 3 topics (this includes any topic designated in the 'Other' area).");
		return false;
	}
	
	if (stringElements.length == 0) {
		alert ("Please list three elements the speaker is prepared to address for each of the topics checked.");
		speakers_form.topElements.focus();
		return false;
	}
	
	if (elementsCharCount > 250) {
		alert ("Please limit the explanation of the elements that the speaker is prepared to address to 250 characters (including spaces).");
		return false;
	}
