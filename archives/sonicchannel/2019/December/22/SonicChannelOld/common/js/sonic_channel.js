...
function bodyId(){
	...
	if(pathArray[2].indexOf("html") != -1){
		idName = "topPage";
	} else if( pathArray[2].indexOf("fanart") ==0 ){
		// ※ファンアートはファイル名で判定
		var idName = pathArray[2];
		if(pathArray[3]==""){
			var fileName = "index";
			//alert(fileName);
		} else if(pathArray[3].indexOf("html") != -1 ){
			// fanartディレクトリ直下用
			var fileName = pathArray[3].replace(".html", "");
			//alert(fileName);
		} else if(pathArray[5].indexOf("html") != -1 || pathArray.length>5){
			//ファンアート過去データ閲覧画面用
			var fileName = pathArray[5].replace(".html", "");
			var fileName = pathArray[3]+pathArray[4];
		}


/*
//		var current_pathname = window.location.pathname;
		var nowPath_cut = nowPath.substr(current_pathname.indexOf("fanart"));
		var arr_nowPath_cut = nowPath_cut.split('/');
alert(current_pathname_cut);
		
		if(pathArray[3]==""){
			var fileName = "index";
		} else if(pathArray[3].indexOf("html") != -1 ){
			if(arr_current_pathname.length >2){
				// ファンアート過去画面の場合
				var fileName = current_pathname_cut.replace(".html", "");
			}else{
				var fileName = pathArray[3].replace(".html", "");
			}
			alert(fileName);
