$(function() {
	...
	$("#impress").load(dataName,function(){
		...
		$('#top-anchor01').colorbox({iframe:true,innerWidth:640,innerHeight:360});
		...
		$('#top-anchor02').colorbox({inline:true,innerWidth:640,innerHeight:480});
		...
		intarvalID2 = setInterval( "blinkBanner()", 50 );
...
function blinkBanner(){
	var $topblink = $('#contents-top #top-blink');
	var blinkopacity  = Math.abs($topblink.css("opacity"));
	blinkopacity += reflector/10;
	if(blinkopacity<=0){
		reflector *= -1
	}else if(blinkopacity>=1){
		reflector *= -1
	}
	$topblink.css({"opacity":blinkopacity})
}
