...
{
	document.tb.submit();
	//alert(document.tb.exportCountry.selectedIndex);	
...
function viewExport()
{
	...
	if (document.tb.Export != null) {
		if (document.tb.Export[0].checked)
			{
				document.getElementById('exportList').style.visibility = 'visible';
			}
			
		else
			{
				document.getElementById('exportList').style.visibility = 'hidden';			
			}			
	}
}
...
function addHandler()
{
	if (document.tb.exportCountry != null) {
		document.tb.exportCountry.onchange = submitForm;
		var countryValue;	
		countryValue = '';
	
		for (var i=0; i < document.tb.exportCountry.length; i++)
			{
				if (document.tb.exportCountry.options[i].value == countryValue)
					{document.tb.exportCountry.options[i].selected = true;}
				if (i == 6)
					{document.tb.exportCountry.options[i].text ='CIS';					
					}
			}				
...
<BODY onLoad="viewExport();addHandler();">
      ...
      <TD>
		...
		<tr>
		   ...
		   <TR>
			  ...
			  <A CLASS="toolbar" HREF="javascript:ListByPavilion();">TechZone</A> | 	      
		      <a href="exhib_product.asp" class="toolbar">Product/Export Region</a>
			...
			<tr><td class=text>
				...
				<select name="productCategory" class=text onChange=submitForm()>
					...
					<option value='1'>Audio</option><option value='416'>Biometrics</option><option value='3'>Blank Media</option><option value='667'>Broadband</option><option value='4'>Computer Hardware and Software</option><option value='452'>Digital Imaging/Video Editing</option><option value='5'>Electronic Gaming</option><option value='1114'>Electronics Clothing & Accessories</option><option value='567'>Embedded Technologies</option><option value='563'>Film/Video Production (Professional Grade)</option><option value='482'>High Performance Audio</option><option value='6'>Home Appliances</option><option value='7'>Home Data Networking</option><option value='637'>Home Healthcare Products</option><option value='534'>Home Theater</option><option value='8'>Integrated Home Systems</option><option value='10'>Mobile Office</option><option value='9'>Mobile/Vehicle Electronics</option><option value='498'>Nanotechnology</option><option value='12'>On-Line/Internet</option><option value='499'>Online Commercial Products and Services</option><option value='561'>Other Consumer Electronics</option><option value='13'>Personal Electronics</option><option value='1044'>Personal Safety and Security Products</option><option value='14'>Photographic Equipment</option><option value='15'>Publications</option><option value='16'>Retail Resource</option><option value='1124'>Robotics</option><option value='17'>Satellite Systems</option><option value='511'>Small Office/Home Office</option><option value='1119'>Sports Electronics</option><option value='1108'>Subscriber Services</option><option value='18'>Telephones</option><option value='19'>Trade Associations</option><option value='20'>Video</option><option value='1061'>Voice Over IP (VoIP) Hardware and Software</option><option value='678'>WiFi</option><option value='21'>Wireless Communications</option>
				</select>				
			</td></tr>
