...
<head><!-- PageID 4321 - published by Open Text Web Solutions 10.1 - 10.1.1.334 - 29635 -->
    ...
    <title>2012 CES: Conference Tracks</title>
...
<ul id="sub-faqs" class="subnav" style="display: none;" onmouseout="setMenuTimer()">
    ...
    <div class="icon-wrap">
        ...
        <a target="_blank" href="http://www.cesweb.org/news/rssNews.asp"><img src="/images/framework/sub_rss.png" alt="rss"></a>
                    ...
                    <div class="right">
                        <div id="breadcrumb">
                            <div><a href="/default.htm" class="footer">Home</a>&nbsp;&gt;&nbsp;<a href="/conferenceProgram/default.asp" class="footer">Conference Program</a>&nbsp;&gt;&nbsp;Conference Tracks</div>
                        </div>
                         <div class="headerbox">
...
<!-- Jay's code starts here --><LINK rel=stylesheet type=text/css href="http://www.cesweb.org/conferenceProgram/resources/SessionGrid.css">
...
<SCRIPT type=text/javascript src="resources/shoppingCart.js"></SCRIPT>

<SCRIPT type=text/javascript src="resources/tracks.js"></SCRIPT>

<SCRIPT type=text/javascript>
            var trackData; var pricingData; var computedCart; var m_sessions = [];
            var shoppingCartList; var output; var m_codeLookup; var fixScroll; 
   // fixScroll is a hack. The browser kept scrolling to the top of the page when I was hiding sessions....dunno why
   function loadExternalScripts() {
    $.ajaxSetup({async: false});
    $.getScript('resources/jquery.cookie.js', function(){
     //alert("Loaded helpers");
    });
    $.getScript('resources/dateFormat.js', function(){
     //alert("loaded dateFormat");
    });
    $.getScript('resources/datejs.js', function(){
     //alert("loaded datajs");
    });
    //$.getScript('resources/tracks.js', function(){
     //alert("Loaded helpers");
    //});
...
var logger = { 
 ...
 debugItem: function( value ) {
  ...
  return ''
    ...
    $('<link />', {
                    href: 'http://2012.cesweb.org/conferenceProgram/resources/sessionGrid.css',
                    media: 'screen',
                    rel: 'stylesheet',
                    type: 'text/css',
                    'class': 'gridCss'
    ...
    // IE doesn't always notice the presence of new stylesheets
                ...
                $.ajax({
                    ...
                    success: function(data) {
      trackData = data;
      $( "#tmplTrackList" ).tmpl( data ).appendTo( '#trackList' );
     },
    ...
    $('.track').live('click', function(event) {
     ...
     var b = $('#TrackInfo').height() + $(this).position().top - $('#inner-content-long').height() + $('#inner-content-long').position().top;
     if ($.browser.safari) {
      b = $('#TrackInfo').height() - $('#inner-content-long').height();
     }
     if ( b>0 ) {
      $('#TrackInfo').css('position', 'relative').css('top', $(this).position().top-b);
     }
...
<DIV style="LINE-HEIGHT: 17px; WIDTH: 720px; FONT-FAMILY: arial; FONT-SIZE: 12px" id=ConferenceTracks>
<DIV style="WIDTH: 160px; FLOAT: left">
<UL id=trackList></UL></DIV>
<DIV style="PADDING-BOTTOM: 5px; PADDING-LEFT: 5px; WIDTH: 540px; PADDING-RIGHT: 5px; FLOAT: right; PADDING-TOP: 5px" id=TrackInfo>Select a track to get started</DIV></DIV>
<SCRIPT id=tmplTrackList type=text/html>
   <li style="padding-top: 15px;">
    {{if Logo.length > 0}}
     <img id="trackImage${ID}" class="track" alt="${Title}" title="${Title$}" width="135" src="${Logo}" />
    {{else}}
     <span id="trackImage${ID}" class="track">${Title}</span>
    {{/if}}
   </li>
  </SCRIPT>

<SCRIPT id=tmplTrackInformation type=text/x-jquery-tmpl>
   <div class="trackDetailTitle">
    {{if Pricing.length > 0}}
     {{tmpl($item.data) "#tmplCart"}}
    {{/if}}
 {{if Logo.length > 0}}
  <img id="trackDetailImage${ID}" class="trackImage" alt="${Title}" title="${Title}" src="${Logo}" /><br />
 {{/if}}
   </div>
   <h2 style="text-align: center;" id="TrackTitle">${Title}</h2>
   <div style="clear: both;"></div>
   {{html Description}}
   <br />
   {{html formatHelpers.displaySponsorLogos(formatHelpers.parseLogos(Sponsor))}}
   {{if Sessions.length > 0}}
    <h3>Session List:</h3>
    {{tmpl(formatHelpers.buildSessionList(Sessions)) "#tmplSessionDate"}}
   {{/if}}
  </SCRIPT>

<SCRIPT id=tmplCart type=text/x-jquery-tmpl>
   <div style="float:right; margin-top:10px; padding-bottom: 10px; padding-left: 10px;">
    {{if formatHelpers.itemInCart(Code)==1}}
   <img style="margin-top: -10px;"   src="resources/images/checkmark.jpg" width="26" alt="added to cart" /><br />
   <a href="cart.htm" alt="view cart">view cart</a>
    {{else formatHelpers.itemInPurchaseOptions(Code).Type==1}}
   <img style="margin-top: -10px;"   src="resources/images/valueAdd.jpg" alt="Value Added Upsell" /><br />
   Upsell triggered<br />
   <a href="cart.htm" alt="view cart">view cart</a>
...
<SCRIPT id=tmplSessionDate type=text/x-jquery-tmpl>
   <div class="searchResultsDateList" id="${DateID}">${SessionDate}</div>
   {{tmpl(Sessions) "#tmplDateTimeTitle"}}
  </SCRIPT>

<SCRIPT id=tmplDateTimeTitle type=text/x-jquery-tmpl>
...
<SCRIPT id=tmplSessionList type=text/x-jquery-tmpl>
   <li>${Title}</li>
  </SCRIPT>

<SCRIPT id=tmplSessionInformation type=text/x-jquery-tmpl>
   <div class="expand" style="text-align: left;" id="SessionDetail${ID}">
    <div class="header">
...
<SCRIPT id=tmplSessionSpeakers type=text/x-jquery-tmpl>
   <li>
    ${FirstName}&nbsp;${LastName}&nbsp;&ndash;&nbsp;${Company}
   </li>
  </SCRIPT>
<INPUT id=json type=hidden> <!-- Jay's code ends here -->
<br/>
