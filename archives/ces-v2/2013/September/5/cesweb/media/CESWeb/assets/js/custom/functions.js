       ...
       $('#blog').ready(function()
        {
                ...
                var d = new Date();
                $.ajax({
                  url: "http://www.ce.org/blog?feed=json",
                  cache:false,
                  dataType: 'jsonp',
                  success: function(data){
                         ...
                         for(var i=0; i<data.posts.length; i++){
                             ...
                             var myDate = item.date.split(" ");
                             if(!item_html[j]) item_html[j]="";
                            if ((i+1) % 5 ==1) item_html[j] += "<div id='blog_" + j + "' style='display:none;'><div id='blog-1'>";
                            item_html[j] += '<div id="blog-date"><p>'+ myDate[0] +' '+ myDate[1] +' '+ myDate[2] +' '+ myDate[3] +' </p></div><h3>'+item.title+'</h3><div id="blog-content"><p>'+item.excerpt+'</div><div id="read-more"><a target="_blank" href="'+item.url+'"> Read More</a></p></div>';
