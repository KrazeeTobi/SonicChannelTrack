jQuery.noConflict();
	...
	var sessionTabs = {
			...
			bindsessionTabs: function(tabControl, dataArray) {
				tabControl.tabs({
					...
					show: function(event, ui) { 
						...
						if ( jQuery.inArray(t, _sessionDates) > 0 ) {
			...
			initializeTabs: function(tabControl) {
				...
				var ap = jQuery.inArray(t, _sessionDates);
	...
	var dateHelpers = {
		getFirstLastSessionTime: function(selectedDate) {
			...
			jQuery.each( $(sessionData).find('session:has(endson:contains("' + selectedDate + '"))') , function() {
	...
	var sessionHelpers = {
		...
		createSpeakerArray: function(speaker, type) {
			var a = [];
			jQuery.each($(speaker).find('speaker[role=' + type + ']'), function(i, s) {
				var o = new Object();
				o.ID = $(s).attr('id');
				o.Role = $(s).attr('role');
		...
		sessionElementToObject: function(s) {
			...
			var sponsorLogos = [];
			jQuery.each($(s).find('sponsors > sponsor > sponsorlogo'), function(i, item) {
		...
		sortSessions: function(sessions, gridStartTime, gridEndTime) {
			...
			jQuery.each(sessions, function(i, s) {
	...
	jQuery.expr[':'].contentIs = function(el, idx, meta) {
	...
	function getShowDates(d) {
		var a = [];
		$(d).each(function()  {  if ( jQuery.inArray( $(this).text().substring(0,10), a) < 0 ) {  a.push($(this).text().substring(0,10));  }  });
		a.sort();
		return a;
	}
	function getTracks(t) {
		// is this the best way to do this? jQuery.inArray doesn't seem to work for objects so I'm building a simple array of id and then pushing the object as I push into the simple array
		var a = []; var oa = [];
		$(t).each(function(i, track) {
			if ( jQuery.inArray( $(track).attr('id'), a) < 0 ) {
	...
	function createSortedArray(v) {
		var a = [];
		$(v).each(function()  {  if ( jQuery.inArray( $(this).text(), a) < 0 && $(this).text().length > 0 ) {  a.push($(this).text());  }  });
		a.sort();
		return a;
	}
	...
	function buildGrid(selectedDate) {
		...
		if (selectedDate == null && selectedDate !== null) { 
			...
			// loop over locations
			jQuery.each( createSortedArray( $(sessionData).find("session").children("startson:contains('" + selectedDate + "')").parent().children("location") ), function (locationIndex, location) {
				var _locationObject = new Object(); var _roomArray = [];
				
				jQuery.each( createSortedArray( $(sessionData).find('session:has(location:contentIs("' + location + '")):has(startson:contains("' + selectedDate + '"))').children("room") ),
...
//                                                        sessionWidth += 602 - totalDuration;
	...
	var formatHelpers = { 
		...
		buildSessionList: function(sessions) {
			...
			jQuery.each($(sessions), function(i, session) {
				if ( jQuery.inArray(dateFormat(session.SessionDate, 'mmmm d, yyyy'), searchResultDatesArray) < 0 ) {
			...
			var a = [];
			jQuery.each(sponsors, function(i, item) {
	...
	function getCategories(search, categories) {
		...
		jQuery.each(categories, function() {
			if (this.Title == search) {
				jQuery.each(this.Subcategories, function(i, track) {
					var o = new Object();
					o.ID = track.ID;
					o.Title = track.Title;
	...
	function getLocations(data) {
		...
		jQuery.each(data.Sessions, function() {
			var o = new Object();
	...
	function buildDetail(id)
		{
		var retHTML = "";
		jQuery.each(sessionData.Sessions, function(sessionCount, session) {
