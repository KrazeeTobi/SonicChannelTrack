...
function response(jsonData) {
    ...
    for (var i=0; i<jsonData["items"].length; i++) {
        ...
        var padDay = strpad(thisDate.getDate() + '');        
var dispMonth = thisDate.getMonth() + 1;
    ...
    $('.title').hover(function() {
        $(this).find('.thumb').find('div').css({
            'background': 'url("/cesweb/media/CESWeb/assets/images/general/CES-play-button.png") no-repeat center center' // OVER
        });
    }, function() {
        $(this).find('.thumb').find('div').css({
            'background': 'url("/cesweb/media/CESWeb/assets/images/general/CES-play-button-transparent.png") no-repeat center center' // OUT
        });
    });
}
$(document).ready(function(){
