...
<head id="head"><title>
	Eureka Park Leaderboard - CES 2017
</title><meta name="description" content="See which CES Eureka Park exhibitors have contributed to raising over $1.03 billion." /> 
...
<meta property="og:url" content="http://www.ces.tech/Events-Programs/Archived-Items/Eureka-Park-Leaderboard/Company-Detail.aspx" />
<meta property="og:title" content="Company Detail" />
...
<meta name="twitter:title" content="Company Detail">
...
<body class="LTR ENUS ContentBody" >
    <form method="post" action="/Events-Programs/Archived-Items/Eureka-Park-Leaderboard/Company-Detail.aspx" id="form">
            ...
            <div class="row">
              <div class="col-sm-40">
                <ul>
                  ...
                  <li><a href="/Events-Experiences.aspx" class="CMSBreadCrumbsLink">Events & Experiences</a> </li><li class="bread-div">|</li><li> <span  class="CMSBreadCrumbsCurrentItem">Company Detail</span>
</li>
...
<link href="/Content/bootstrap-social-icons/font-awesome.css" rel="stylesheet">
<link href="/Content/bootstrap-social-icons/bootstrap-social.css" rel="stylesheet">

<script type="text/javascript" src="/CES/js/plugins/underscore-1.8.3/underscore-min.js"></script>
<script type="text/javascript" src="/CorporateSite/media/assets/plugins/mustache/mustache.js"></script>
<script type="text/javascript" src="/CorporateSite/media/assets/plugins/moments-2.5.1/moments.js"></script>
...
<script>
    jQuery(document).ready(function ($) {
        var permalink = qs["permalink"];
        $.when(
			    $.ajax('/cea_cms_items/handlers/ces/CrunchBase/GetCrunchBaseOrganization.ashx?permalink={0}'.format(permalink)),
			    $.ajax('/cea_cms_items/handlers/ces/CrunchBase/GetCrunchBaseInvestorCount.ashx?permalink={0}'.format(permalink)),
			    $.ajax('/cea_cms_items/handlers/ces/CrunchBase/GetEurekaParkBoothInfo.ashx?permalink={0}'.format(permalink)),
                $.getJSON('/CEA_CMS_Items/Handlers/getTransformationTemplate.ashx?callback=?', { templateName: "iCes.Transformations.EurkaPark.CrunchBase.Detail.mustache" })
		    ).then(function (organization, investorCount, epBoothInfo, template) {
		        var data = _.first(organization).data.properties;
		        var relationships = _.first(organization).data.relationships;
		        data.formattedLocation = formatLocation(relationships.headquarters.item.properties);
		        //data.funding_raised = (data.total_funding_usd).formatMoney(2);
		        data.funding_raised = nFormatter(data.total_funding_usd, 2);
		        data.funding_rounds = _.first(organization).data.relationships.funding_rounds.paging.total_items;
		        data.total_investors = _.first(investorCount);
		        data.founders_list = _.map(relationships.founders.items, function (item, key) { return "{0} {1}".format(item.properties.first_name, item.properties.last_name); }).join(", ");
		        data.categories_list = _.map(relationships.categories.items, function (item, key) { return item.properties.name; }).join(", ");
		        data.social_list =
                    _.map(
                        // first bit filters out the websites that we care about
		                _.filter(relationships.websites.items, function (item) {
		                    return (["facebook", "google+", "twitter", "linkedin"]).indexOf(item.properties.website_type) > -1;
		                }),
                    // this bit flattens the object down. this isn't necessary really.
                    function (val, key) {
                        return {
                            social_network: val.properties.website_type, url: val.properties.url
                        };
                    });
		        data.news = _.sortBy(
                    _.map(relationships.news.items, function (val, key) {
                        return {
                            'url' : val.properties.url,
                            'title' : val.properties.title.split('|')[0],
                            'publication' : _.isUndefined(val.properties.title.split('|')[1]) ? "" : "{0} - ".format(val.properties.title.split('|')[1]),
                            'posted_date' : moment(val.properties.posted_on).format('MMMM Do YYYY'),
                            'sort_date': moment(val.properties.posted_on).format('YYYYMMDD'),
                            'author' : val.properties.author
                        };
                    }), function (o) {
                        return -o.sort_date; // the negative sign (-) orders in reverse (descending)
                    });
		        data.eureka_park_booth_info = _.first(epBoothInfo);
		        $("#insertHere").html(Mustache.render(_.first(template), data));
		    }).fail(function (a, b, c) {
                debugger;
            });
    });
    ...
    function formatLocation(data) {
        ...
        if (data.country == "United States" || data.country == "Canada")
            return data.city + ", " + data.region_code2 + ", " + data.country;
        else
            return data.region_code2 + ", " + data.country;
    }
...
(function() {var fn = function() {$get("manScript_HiddenField").value = '';Sys.Application.remove_init(fn);};Sys.Application.add_init(fn);})();//]]>
  ...
  <input type="hidden" name="__VIEWSTATE" id="__VIEWSTATE" value="" />
  <script type="text/javascript"> 
      //<![CDATA[
