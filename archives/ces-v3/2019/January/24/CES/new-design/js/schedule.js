(function ($) {
    ...
    function elScroll(e) {
        ...
        if(window.pageYOffset > pageHeight * pageNo * 3/4) {// start at 3/4 of the way through the page
            getNextPage();
        }
    }
    ...
    function getPageHeight() {
        ...
        const firstElement = document.querySelector(elementSelector + ':first-of-type');
        let lastElement = document.querySelector(elementSelector + ':nth-of-type(' + pageSize + ')');

        if(!lastElement) {
            lastElement = document.querySelector(elementSelector + ':last-of-type');
        }
    ...
    load(filters, isAddToAgendaEnabled);
