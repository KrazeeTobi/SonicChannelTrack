...
(function ($) {
    ...
    function loadCart() {
        ...
        $.when(
            ...
            debugger;
            if (response.responseText == "Cart does not exist") {
                //$(body).append(Mustache.render(Template.get("unpurchaseable"), _unpAddToCartObj));
                if ($('div#doesNotExistModal').length == 0) {
                    var m = Mustache.render(Template.get("doesNotExist"), {});
                    $("body").append(m);
                }
                $("#doesNotExistModal").modal('show');
                $.cookie("ShoppingCartUser", null);
                $.removeCookie('ShoppingCartUser', { path: '/' });
                loadCart();
            }
    ...
    function bindAddToCartButton() {
        ...
        $('body').on('click','.addToCart',function () {
            ...
            { id = $(this).parent().attr('id').replace('addToCart', ''); }
            else
            { id = $(this).attr('id').replace('addToCart', ''); }
            Cart.addToCart(id);
    ...
    function updateItemStatus(code) {
        ...
        var pricingDomItem = jQuery('#pricingLine' + code);
        jQuery(domItem).html('');
        ...
        // adding unit price to pricing line
        if (_.isNumber(_pricingInfo.UnitPrice) && !isNaN(_pricingInfo.UnitPrice)) {
            jQuery(pricingDomItem).html(_pricingInfo.UnitPrice);
        }
    ...
    function updateWaysToBuy(code, domItem) {
        ...
        if (_.isUndefined(_unpAddToCartObj) || _.isUndefined(_unpAddToCartObj.Items) || _unpAddToCartObj.Items.length == 0)
            return;
