(function ($) {
    ...
    function renderPage(configureMyAgendaBtn) {
        ...
        $scheduleFilterCollapseSessionType.html(CES.templates.scheduleFilters({ filters: scheduleData.filters.types, apiName: "type", subApiName: "type" }));
        ...
        $scheduleFilterCollapseLocation.html(CES.templates.scheduleFilters({ filters: scheduleData.filters.locations, apiName: "locations", subApiName: "buildings" }));
        ...
        $scheduleFilterCollapseSessionType.find("input:checkbox").change(filterRelativesChange);
    ...
    function clearTimes() {
        // This clears the slider
        var options = $scheduleFilterRange.slider("option");
        $scheduleFilterRange.slider("values", [options.min, options.max]);

        // This sets the Start and End time to 8:00AM and 11:00PM respectively
        $scheduleFilterTime1.html('8:00 AM');
        $scheduleFilterTime2.html('11:00 PM');
    }
    ...
    function clearFiltersClick(event) {
        ...
        event.preventDefault();
        $scheduleFiltersContainer.find("input:checkbox").prop('checked', false);
    ...
    var $scheduleFilterCollapseSessionType = $("#schedule-filter-collapse-sessionType-xl");
    ...
    //Pull out requested types in url if it exists
    ...
    if (types) {
        ...
        filters.type = typesArray;
    }

    if (locations) {
        var locationsArray = locations.split(",");
        filters.locations = locationsArray;
    }

    if (dates) {
        var datesArray = dates.split(",");
        filters.dates = datesArray;
    }

    if (track) {
        var trackArray = track.split(",");
        filters.track = trackArray;
    }
