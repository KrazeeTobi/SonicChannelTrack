        ...
        query = Object.keys(args).map(function(key){
                if (args[key] instanceof Array) {
                    ...
                    args[key].forEach(function(val, index){
                        query += [key, val].map(encodeURIComponent).join("=");
                        if (index !== args[key].length -1) {
                            query += '&';
                        }
                    });
                    return query;
...
console.log(request_url);
    ...
    view_exhibit: function(json, self, display_area, lang) {
        ...
        Object.keys(json.exhibits).forEach(function(index) {
            ...
            if(exhibit['image']){
                html.find('.exhibit-img').append('<img src="/tgs/2021/ems_data/'+exhibit['image']+'" alt="'+exhibit['name']+'">');
            }
        ...
        Object.keys(json['exhibit-items']).forEach(function(index) {
            ...
            Object.keys(exhibit_items).forEach(function(index2) {
                ...
                */}).toString().match(/(?:\/\*(?:[\s\S]*?)\*\/)/).pop().replace(/^\/\*/, "").replace(/\*\/$/, "");
                ...
                //ジャンルのタグ化（日・英共通）
                var genre_all = [exhibit_item['genre1'],exhibit_item['genre2']];
                var genre = genre_all.filter(checkEmpty);
